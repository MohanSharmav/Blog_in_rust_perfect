...
/// SQLite-backed adapter implementing the [`PostRepository`] port.
#[derive(Clone)]
...
impl PostRepository for SqlitePostRepository {
    ...
    async fn page(&self, page_number: i32, per_page: i64) -> Result<Vec<Post>> {
        Ok(sqlx::query_as::<_, Post>(
            "select id, title, description from posts order by id asc limit ?1 offset (?2-1)*?1",
        )
        .bind(per_page)
        .bind(page_number)
        .fetch_all(&self.pool)
    ...
    async fn create_with_category(&self, new_post: &NewPost, category_id: i32) -> Result<()> {
        let result = sqlx::query("insert into posts(title,description) values (?1,?2)")
            .bind(&new_post.title)
            .bind(&new_post.description)
            .execute(&self.pool)
            .await?;
        ...
        sqlx::query("insert into categories_posts(post_id,category_id) values (?1,?2)")
            .bind(id)
            .bind(category_id)
            .execute(&self.pool)
            .await?;
