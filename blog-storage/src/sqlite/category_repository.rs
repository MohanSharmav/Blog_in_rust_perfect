...
/// SQLite-backed adapter implementing the [`CategoryRepository`] port.
#[derive(Clone)]
...
impl CategoryRepository for SqliteCategoryRepository {
    async fn all(&self) -> Result<Vec<Category>> {
        Ok(
            sqlx::query_as::<_, Category>("select id, name from categories")
                .fetch_all(&self.pool)
                .await?,
        )
    }
    ...
    async fn all_except(&self, id: i32) -> Result<Vec<Category>> {
        Ok(
            sqlx::query_as::<_, Category>("select id, name from categories where not id=?1")
                .bind(id)
                .fetch_all(&self.pool)
                .await?,
        )
    ...
    async fn page(&self, page_number: i32, per_page: i32) -> Result<Vec<Category>> {
        Ok(sqlx::query_as::<_, Category>(
            "select id, name from categories order by id asc limit ?2 offset (?1-1)*?2",
        )
        .bind(page_number)
        .bind(per_page)
        .fetch_all(&self.pool)
