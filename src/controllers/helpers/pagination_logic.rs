pub async fn index_pagination(
    ...
    for index in 1..total_pages_count + 1 {
        ...
        // if 2 = [1,2,3]
        // 2==2 then active
        let tag_and_url = if index == current_page {
            r#"<a class="active"  href="/posts/page/"#
        } else {
            r#"<a style="margin: 0 4px;" href="/posts/page/"#
        };
        pagination.push_str(tag_and_url);
        let href_link = index.to_string();
        pagination.push_str(&href_link);
        let end_of_tag = r#"">"#;
        pagination.push_str(end_of_tag);
        let text_inside_tag = index.to_string();
        pagination.push_str(&text_inside_tag);
        let close_tag = r#"</a>"#;
        pagination.push_str(close_tag);
    }
...
pub async fn general_category(
    ...
    for index in 1..total_pages_count + 1 {
        let tag_and_url = if index == current_page {
            r#"<a class="active"  href="/posts/category/"#
        } else {
            r#"<a style="margin: 0 4px;" href="/posts/category/"#
        };
...
pub async fn admin_posts_categories(
    ...
    for index in 1..=total_pages_count {
